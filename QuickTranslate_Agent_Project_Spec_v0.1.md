...
| **模式** | **初始区域**             | **边界策略**                                             |
|----------|--------------------------|----------------------------------------------------------|
| Word     | 约 15 词宽 × 4 行高（按行高估算起捕，选词触边时用实际行高重算并最多重抓 1 次） | 以鼠标为中心，转换为当前显示器物理像素并裁剪到 Monitor；截图后立即用虚线预览框标出范围。 |
| Block    | 约 1200×720 DIP 等效区域 | 若候选文本块触碰截图边缘，允许扩大区域并最多重试 1 次。  |

上述尺寸是初始调参值，最终以 Benchmark 为准。禁止每次默认截取整块 4K 屏幕。显示策略：选定框在 Popup 出现后约 3 秒自动收起；翻译 Popup 约 5 秒后自动消失（用户点击 Popup 交互则常驻）；Esc 可随时提前关闭。
...
| **启发式**      | **初始建议**               | **说明**                            |
|-----------------|----------------------------|-------------------------------------|
| Vertical gap    | ≤ 0.5 × median line height | 用于排除明显段落间距/标题间距。     |
| Height ratio    | 0.65 - 1.50                | 防止把大标题/小脚注误并入正文。     |
| X overlap       | ≥ 0.5 或左边缘近似         | 适配换行长度不同的正文。            |
| Left edge delta | ≤ 1.5 × line height        | 列表/缩进场景需后续调参。           |
| Max lines       | ≤ 30                       | 限制单块行数，防止密集排版过度生长。|
| Edge retry      | 最多 1 次，扩大 ×1.4       | 文本块触碰 Capture 边界时扩大截图。 |
